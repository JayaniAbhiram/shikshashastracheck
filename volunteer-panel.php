      ...
      if ($appdate1 < strtotime($oneMonthFromNow)) {
          if (date("Y-m-d", $appdate1) >= $cur_date) {
              if ((date("Y-m-d", $appdate1) == $cur_date && date("H:i:s", $apptime1) > $cur_time) || date("Y-m-d", $appdate1) > $cur_date) {
                  ...
                  // Check if the booking is already confirmed at this slot
                  ...
                  if (mysqli_num_rows($check_query) == 0) {
                      // Insert the booking if the slot is available
                      $query = mysqli_query($con, "INSERT INTO book(pid, fname, lname, gender, email, contact, community, appdate, apptime, userStatus, communityStatus) 
                        VALUES ($pid, '$fname', '$lname', '$gender', '$email', '$contact', '$community', '$appdate', '$apptime', '1', '1')");
                      ...
                      echo "<script>alert('This time slot is already confirmed for another booking. Please choose another slot!');</script>";
                  }
              } else {
...
<body>
  ...
  <div class="section-container" id="sections">
    ...
    <div style="background: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); width: 90%; max-width: 600px;">
        ...
        <form action="volunteer-panel.php" method="POST" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 20px; animation: formAppear 1.5s ease;">
                <label for="name" style="font-weight: bold; color: #333; font-family: 'Roboto', sans-serif;">Name:</label>
                <input type="text" name="name" id="name" required style="border: 2px solid #ddd; border-radius: 8px; padding: 12px; font-family: 'Roboto', sans-serif; transition: border-color 0.3s ease;">
                
                <label for="gmail" style="font-weight: bold; color: #333; font-family: 'Roboto', sans-serif;">Gmail:</label>
                <input type="email" name="gmail" id="gmail" required style="border: 2px solid #ddd; border-radius: 8px; padding: 12px; font-family: 'Roboto', sans-serif; transition: border-color 0.3s ease;">
                
                <label for="file1" style="font-weight: bold; color: #333; font-family: 'Roboto', sans-serif;">Choose File 1 (Aadhar Card):</label>
                <input type="file" name="file1" id="file1" required style="border: 2px solid #ddd; border-radius: 8px; padding: 12px; font-family: 'Roboto', sans-serif;">
                
                <label for="file2" style="font-weight: bold; color: #333; font-family: 'Roboto', sans-serif;">Choose File 2 (12th Mark Sheet):</label>
                <input type="file" name="file2" id="file2" required style="border: 2px solid #ddd; border-radius: 8px; padding: 12px; font-family: 'Roboto', sans-serif;">
                
                <label for="file3" style="font-weight: bold; color: #333; font-family: 'Roboto', sans-serif;">Choose File 3 (Graduation Certificate):</label>
                <input type="file" name="file3" id="file3" required style="border: 2px solid #ddd; border-radius: 8px; padding: 12px; font-family: 'Roboto', sans-serif;">

                <button type="submit" name="upload" style="background-color: #6A00FF; color: #fff; border: none; border-radius: 8px; padding: 14px 24px; cursor: pointer; font-size: 18px; font-family: 'Poppins', sans-serif; transition: background-color 0.3s ease, transform 0.2s ease; text-transform: uppercase;">
                    Upload Files
                </button>
            </form>
        ...
        if (isset($_POST['upload'])) {
            ...
            if ($stmtCheck->num_rows > 0) {
                echo "<script>Swal.fire('Oops!', 'Documents already uploaded with this name or Gmail.', 'error');</script>";
            } else {
                ...
                if ($stmtInsert->execute()) {
                    echo "<script>Swal.fire('Success!', 'Documents successfully submitted and inserted into the database.', 'success');</script>";
                } else {
                    echo "<script>Swal.fire('Error!', 'Failed to insert data into the database. Please try again.', 'error');</script>";
                }
