...
if(isset($_POST['prescribe']) && isset($_POST['pid']) && isset($_POST['AppID']) && isset($_POST['appdate']) && isset($_POST['apptime']) && isset($_POST['lname']) && isset($_POST['fname'])){
  ...
  $query=mysqli_query($con,"insert into feedback(community,pid,AppID,fname,lname,appdate,apptime,feedpoints,feedback) values ('$community','$pid','$AppID','$fname','$lname','$appdate','$apptime','$feedpoints','$feedback')");
    if($query)
    {
      echo "<script>alert('Feedback Submitted successfully!');window.location.href = 'community-panel.php';</script>";
    }
    else{
    ...
    <div class="form-box">
      <form class="form-group" name="prescribeform" method="post" action="feedback.php">
        ...
        <div class="row">
          ...
          <div class="cont">
    <input id="feedpoints" type="number" min="1" step="1" name="feedpoints" required
